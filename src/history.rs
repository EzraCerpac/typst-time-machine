    ...
    fn collect_revisions(
        ...
        for revision in &mut revisions {
            revision.bookmarks = bookmark_map
                ...
                .cloned()
                .unwrap_or_default();
            revision.changed_paths = self.changed_paths(revision)?;
        }
    ...
    fn jj_bookmarks(
        ...
        let output = run_text(
            Command::new("jj")
                ...
                .arg(limit.to_string())
                .args(["--no-graph", "-T"])
                .arg("commit_id ++ \"\\t\" ++ bookmarks ++ \"\\n\""),
            "read JJ bookmarks",
        )?;
        Ok(parse_bookmark_lines(&output))
    }

    fn changed_paths(&self, revision: &Revision) -> Result<Vec<PathBuf>> {
        let mut command = Command::new("git");
        command.arg("--git-dir").arg(&self.git_dir).args([
            "diff-tree",
            "--no-commit-id",
            "--name-only",
            "-r",
        ...
        if let Some(parent) = revision
            .parent_ids
            .first()
            .filter(|parent| !is_root_id(parent))
        {
            command.arg(parent).arg(&revision.commit_id);
        } else {
            command.arg("--root").arg(&revision.commit_id);
        }
        let output = run(&mut command, "read changed paths")?;
        Ok(output
            .stdout
            .split(|byte| *byte == 0)
